...
int height ) const {
	...
	for ( int y = 0; y < height; ++y ){
		for ( int x = 0; x < width; ++x ){
			int DataPlace = (y + srcY) * mWidth + (x + srcX);
			int VarmPlace = (y + dstY) * windowWidth + (x + dstX);
			if (DataPlace >= 0 && VarmPlace >= 0) {
				unsigned src = mData[DataPlace];
				unsigned* dst = &vram[VarmPlace];
				*dst = blend(src, *dst);
			}
		}
	}
